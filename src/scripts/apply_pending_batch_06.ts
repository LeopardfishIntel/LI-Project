...
const batchPath = path.resolve(process.cwd(), 'src/data/pending_batch_06.json');
...
async function applyBatch06() {
  console.log('🚀 Applying Batch 06 to Canonical Dataset & Firestore...');

  const batch06: any[] = JSON.parse(fs.readFileSync(batchPath, 'utf-8'));
  ...
  for (const item of batch06) {
    ...
    school.salary_confidence = item.salary_benchmark_category === 'STRONG_MARKET_EVIDENCE' ? 'High' : 'Medium';
...
applyBatch06().catch(err => {
  console.error('❌ Failed to apply batch 06:', err);
  process.exit(1);
});
