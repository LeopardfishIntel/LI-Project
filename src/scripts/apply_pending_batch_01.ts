...
const batchPath = path.resolve(process.cwd(), 'src/data/pending_batch_01.json');
...
async function applyBatch01() {
  console.log('🚀 Applying Batch 01 to Canonical Dataset & Firestore...');

  const batch01: any[] = JSON.parse(fs.readFileSync(batchPath, 'utf-8'));
...
applyBatch01().catch(err => {
  console.error('❌ Failed to apply batch 01:', err);
  process.exit(1);
});
