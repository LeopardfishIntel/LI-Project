...
const batchPath = path.resolve(process.cwd(), 'src/data/pending_batch_10.json');
...
async function applyBatch10() {
  console.log('🚀 Applying Batch 10 to Canonical Dataset & Firestore...');

  const batch10: any[] = JSON.parse(fs.readFileSync(batchPath, 'utf-8'));
...
applyBatch10().catch(err => {
  console.error('❌ Failed to apply batch 10:', err);
  process.exit(1);
});
