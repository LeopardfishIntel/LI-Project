...
const batchPath = path.resolve(process.cwd(), 'src/data/pending_batch_04.json');
...
async function applyBatch04() {
  console.log('🚀 Applying Batch 04 to Canonical Dataset & Firestore...');

  const batch04: any[] = JSON.parse(fs.readFileSync(batchPath, 'utf-8'));
...
applyBatch04().catch(err => {
  console.error('❌ Failed to apply batch 04:', err);
  process.exit(1);
});
