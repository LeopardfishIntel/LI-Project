...
const batchPath = path.resolve(process.cwd(), 'src/data/pending_batch_08.json');
...
async function applyBatch08() {
  console.log('🚀 Applying Batch 08 to Canonical Dataset & Firestore...');

  const batch08: any[] = JSON.parse(fs.readFileSync(batchPath, 'utf-8'));
...
applyBatch08().catch(err => {
  console.error('❌ Failed to apply batch 08:', err);
  process.exit(1);
});
