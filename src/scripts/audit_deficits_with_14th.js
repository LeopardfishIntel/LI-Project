...
async function auditDeficitsWith14th() {
  ...
  for (const s of schools) {
    ...
    const surplusBaseUSD = Math.round(netUSD - outgoingsUSD);
    const surplusBasePercent = netUSD > 0 ? parseFloat(((surplusBaseUSD / netUSD) * 100).toFixed(1)) : -100;

    // Check 14-month or 13-month statutory cycle
    const is14Mo = ['spain', 'portugal', 'italy', 'austria', 'greece'].includes(canonicalCountry(s.country));
    const effectiveAnnualizedNetUSD = is14Mo ? Math.round(netUSD * (14 / 12)) : netUSD;
    const effectiveSurplusUSD = Math.round(effectiveAnnualizedNetUSD - outgoingsUSD);
    const effectiveSurplusPercent = parseFloat(((effectiveSurplusUSD / effectiveAnnualizedNetUSD) * 100).toFixed(1));
    ...
    results.push({
      ...
      is14Mo,
      effectiveAnnualizedNetUSD,
      outgoingsUSD,
      surplusBaseUSD,
      surplusBasePercent,
      effectiveSurplusUSD,
      effectiveSurplusPercent,
      isDeficit: effectiveSurplusUSD < 0
    });
  }

  const deficitSchools = results.filter(r => r.isDeficit);
  const positiveSchools = results.filter(r => !r.isDeficit);

  console.log(`================ AUDIT RESULTS: DEFICIT CHECK (459 SCHOOLS) ================`);
  console.log(`Total Universe: ${results.length} schools`);
  console.log(`Schools in POSITIVE Surplus: ${positiveSchools.length} (${((positiveSchools.length / results.length) * 100).toFixed(1)}%)`);
  console.log(`Schools in TRUE DEFICIT (< $0): ${deficitSchools.length} (${((deficitSchools.length / results.length) * 100).toFixed(1)}%)\n`);

  console.log(`=== COMPLETE LIST OF TRUE DEFICIT SCHOOLS (${deficitSchools.length}) ===`);
  for (const s of deficitSchools) {
    console.log(`[${s.id}] ${s.name} (${s.country} - ${s.city})`);
    console.log(`   Base Salary: ${s.currency} ${s.localSalary.toLocaleString()} (~$${s.netUSD} USD/mo)`);
    console.log(`   Effective Net (with 13/14th if appl): ~$${s.effectiveAnnualizedNetUSD} USD/mo`);
    console.log(`   Outflows: ~$${s.outgoingsUSD} USD/mo | True Deficit: ~$${s.effectiveSurplusUSD} USD/mo (${s.effectiveSurplusPercent}%)`);
    console.log(`   CoL Doc Matched: ${s.colMatched}`);
  }

  return { deficitSchools, total: results.length, positiveCount: positiveSchools.length };
}

auditDeficitsWith14th().catch(console.error);
