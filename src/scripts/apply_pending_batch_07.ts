...
const batchPath = path.resolve(process.cwd(), 'src/data/pending_batch_07.json');
...
async function applyBatch07() {
  console.log('🚀 Applying Batch 07 to Canonical Dataset & Firestore...');

  const batch07: any[] = JSON.parse(fs.readFileSync(batchPath, 'utf-8'));
...
applyBatch07().catch(err => {
  console.error('❌ Failed to apply batch 07:', err);
  process.exit(1);
});
