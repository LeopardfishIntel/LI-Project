...
const batchPath = path.resolve(process.cwd(), 'src/data/pending_batch_05.json');
...
async function applyBatch05() {
  console.log('🚀 Applying Batch 05 to Canonical Dataset & Firestore...');

  const batch05: any[] = JSON.parse(fs.readFileSync(batchPath, 'utf-8'));
...
applyBatch05().catch(err => {
  console.error('❌ Failed to apply batch 05:', err);
  process.exit(1);
});
