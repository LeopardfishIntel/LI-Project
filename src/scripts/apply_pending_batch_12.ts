...
const batchPath = path.resolve(process.cwd(), 'src/data/pending_batch_12.json');
...
async function applyBatch12() {
  console.log('🚀 Applying Batch 12 (FINAL) to Canonical Dataset & Firestore...');

  const batch12: any[] = JSON.parse(fs.readFileSync(batchPath, 'utf-8'));
  ...
  console.log(`\n================ BATCH 12 (FINAL) APPLIED ================`);
  console.log(`Records Applied: ${appliedCount}`);
  console.log(`Total Populated Benchmarks: ${totalPopulated} / ${updatedList.length}`);
  console.log(`Remaining PENDING Schools: ${remainingPending}`);
  console.log(`==========================================================\n`);
}

applyBatch12().catch(err => {
  console.error('❌ Failed to apply batch 12:', err);
  process.exit(1);
});
