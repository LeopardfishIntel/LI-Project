...
const batchPath = path.resolve(process.cwd(), 'src/data/pending_batch_03.json');
...
async function applyBatch03() {
  console.log('🚀 Applying Batch 03 to Canonical Dataset & Firestore...');

  const batch03: any[] = JSON.parse(fs.readFileSync(batchPath, 'utf-8'));
...
applyBatch03().catch(err => {
  console.error('❌ Failed to apply batch 03:', err);
  process.exit(1);
});
