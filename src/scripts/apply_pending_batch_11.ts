...
const batchPath = path.resolve(process.cwd(), 'src/data/pending_batch_11.json');
...
async function applyBatch11() {
  console.log('🚀 Applying Batch 11 to Canonical Dataset & Firestore...');

  const batch11: any[] = JSON.parse(fs.readFileSync(batchPath, 'utf-8'));
...
applyBatch11().catch(err => {
  console.error('❌ Failed to apply batch 11:', err);
  process.exit(1);
});
