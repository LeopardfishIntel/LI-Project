...
const batchPath = path.resolve(process.cwd(), 'src/data/pending_batch_02.json');
...
async function applyBatch02() {
  console.log('🚀 Applying Batch 02 to Canonical Dataset & Firestore...');

  const batch02: any[] = JSON.parse(fs.readFileSync(batchPath, 'utf-8'));
...
applyBatch02().catch(err => {
  console.error('❌ Failed to apply batch 02:', err);
  process.exit(1);
});
