...
export async function getTacticalBriefing(input: z.infer<typeof TacticalBriefingInputSchema>) {
  try {
    ...
    Each of the 4 paragraphs must be very long, rich in qualitative facts, highly descriptive, and separated by empty line breaks.
    ...
    - Strictly NO bullet points, NO lists, NO headers. Output exactly four long, meaty, highly insightful paragraphs separated by empty lines (newlines).
    `;
    ...
    return {
      briefing: (briefingText + additionalAdvisory).replace(/\\n/g, '\n'),
      currentHead,
      ownership
    };
  } catch (error) {
