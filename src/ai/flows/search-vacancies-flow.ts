...
${tier1Queries.map(q => `- ${JSON.stringify(q)}`).join('\n')}`
      ...
      {
        name: "Phase 2: Dedicated TES & Guardian Jobs Sweep",
...
Provide ONLY the raw JSON object.`,
    ...
    const getNormalizedComparisonKey = (title: string): string => {
      ...
      if (input.city) key = key.split(input.city.toLowerCase()).join('');
      if (input.country) key = key.split(input.country.toLowerCase()).join('');
