...
import { 
  ...
  AlertTriangle, AlertCircle, Activity, Clock, Wallet, Banknote, ArrowLeft, FileText, Info, Car, Bus, Lock
} from 'lucide-react';
...
function DecoderContent() {
  ...
  const activeCOL = useMemo(() => {
    ...
    const norm = (s: string) => s?.toLowerCase().replace(/republic/g, 'rep').replace(/\./g, '').trim();
    const sCity = activeSchool.city?.toLowerCase().trim();
    const sCountry = norm(activeSchool.country);
    return costOfLiving.find((c: any) => c.city?.toLowerCase().trim() === sCity) ||
           costOfLiving.find((c: any) => norm(c.country) === sCountry);
  }, [activeSchool, costOfLiving]);

  const activeReq = useMemo(() => {
    if (!activeSchool || !requirements) return null;
    const country = activeSchool.country?.toLowerCase().trim();
    return requirements.find((r: any) => r.id === country || r.country?.toLowerCase() === country);
  ...
  const analysis = useMemo(() => {
    ...
    else if (status === "Married (sole earner)") { personCount = 2; scalar = 1.9; pKey = "single"; }
    ...
    else if (status === "Family +2") { personCount = 4; scalar = 2.65; pKey = "family1Child"; }
    else if (status === "Family +3") { personCount = 5; scalar = 3.0; pKey = "family1Child"; }
    ...
    const rentKey = status === "Single" ? 'rent1br' : (status.includes("Family") ? 'rent3br' : 'rent2br');
    const rentCost = housingStatus?.includes('provided') ? 0 : usdToLocal(safeParse(activeCOL[rentKey] || activeCOL.rent1br));
    const groceriesCost = usdToLocal(safeParse(activeCOL.groceries)) * scalar;
    const utilitiesCost = usdToLocal(safeParse(activeCOL.utilities)) * (scalar * 0.8);
    const connectivityCost = usdToLocal(safeParse(activeCOL.internet) + (safeParse(activeCOL.mobilePhone) * personCount));
    
    const mapType = transportMode === "P" ? "publicTransport" : "carPurchase";
    const transportVal = (activeCOL[mapType] && typeof activeCOL[mapType] === 'object') ? safeParse(activeCOL[mapType][pKey] || activeCOL[mapType].single) : (safeParse(activeCOL.transport) * personCount);
    const transportCost = usdToLocal(transportVal);
    const socialCost = usdToLocal(safeParse(activeCOL.diningSocial)) * scalar;
    ...
    return { 
      ...
      totalIn, totalOut, surplus, rateOfSaving, housingStatus, currency, reliability: activeCOL.dataReliabilityScore 
    };
  }, [activeSchool, activeCOL, settings, responsibilityAllowance, manualAdjustments, extraIncome, currency, transportMode]);
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex justify-between items-center pt-2 border-t border-emerald-500/10">
                         <span className="text-[10px] font-black text-emerald-500 uppercase italic tracking-widest">Total monthly inflow</span>
                      ...
                      <div className="bg-[#f97316]/5 p-6 border border-[#f97316]/20 text-right rounded-sm relative shadow-inner">
                        ...
                        <p className="text-[10px] font-black text-[#f97316] uppercase tracking-[0.2em] mb-1 italic">Disposable surplus</p>
                        <div className="flex flex-col items-end">
                          ...
                          <span className="text-xl font-black text-emerald-500 italic">({analysis?.rateOfSaving}%)</span>
                        </div>
                      </div>
                    </div>
                  </div>
