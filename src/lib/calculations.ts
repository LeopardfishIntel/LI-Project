...
export function calculateBudget(params: BudgetParams) {
  const { calcStatus, selectedSchool, cityData, countryIntel, doYouDrive, setupDays, currency, monthlyCommitments = 0 } = params;
  ...
  else if (calcStatus === 'family-1')       { personCount = 3; scalar = 2.3; }
  else if (calcStatus === 'family-2')       { personCount = 4; scalar = 2.65; }
  else if (calcStatus === 'family-3')       { personCount = 5; scalar = 3.0; }
  ...
  const docsVal = usdToDisplay(heuristic.base + (personCount - 1) * heuristic.perDependent);

  // 2. Rent & Deposit
  const rentKey = calcStatus === 'single' ? 'rent1br' : (calcStatus.includes('family-2') || calcStatus.includes('family-3') ? 'rent3br' : 'rent2br');
  let rentVal = usdToDisplay(safeParse(targetData?.[rentKey] || targetData?.rent1br || 2000)) * 2.5; 
  ...
  const livingVal = (groceriesMonthly + utilitiesMonthly) * setupMultiplier;
  ...
  const transportVal = usdToDisplay(getVal(transportMap, profileKey, doYouDrive ? 1 : personCount));
  ...
  return { 
    ...
    total: docsVal + rentVal + livingVal + transportVal + commitmentsVal,
