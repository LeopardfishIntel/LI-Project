...
export function CostOfLivingCalculator({ school }: CostOfLivingCalculatorProps) {
  ...
  const calculateTotal = () => {
    ...
    if (!school.intel.housing.provided) {
        rentCost = getRentForFamily(costOfLiving, familyStatus).rent;
    }
  ...
  const { rent: rentToShow, label: rentLabel } = getRentForFamily(costOfLiving, familyStatus);
      ...
      <CardContent>
        ...
        <div className="space-y-2 text-sm text-muted-foreground mb-6">
          <div className="flex justify-between items-center">
            ...
            <span>{school.intel.housing.provided ? 'Provided' : formatCurrency(convert(rentToShow), currency)}</span>
          </div>
           <div className="flex justify-between items-center">
