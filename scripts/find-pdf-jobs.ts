...
async function findPdfJobs() {
  const db = getAdminDb();
  console.log('🔍 Searching for PDF job links in featured_jobs_cache...\n');
  ...
  cacheSnap.docs.forEach((doc: any) => {
    ...
    if (url.endsWith('.pdf') || url.includes('.pdf?') || url.includes('/pdf/')) {
      console.log(`PDF Document ID: ${doc.id}`);
...
findPdfJobs().catch(console.error);
