...
import { Calculator, Home, Utensils, TramFront, Zap, Wifi, Smartphone, Coffee, Stethoscope, LineChart, Award, Pencil, Users, Loader2, ShieldAlert } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <Card className="glass border-primary/20">
            <CardHeader>
              <CardTitle className="text-lg">Decoder Settings</CardTitle>
            </CardHeader>
          ...
          <Card className="bg-destructive/10 border-destructive/20">
            ...
            <CardContent>
              <p className="text-xs text-muted-foreground">Always verify if the salary quoted is 'Net' (take-home) or 'Gross'. In many regions, social security can take up to 15% of the headline figure.</p>
            </CardContent>
          </Card>
        </div>

        {/* Decoder View: Dual-Pane Logic */}
        <div className="lg:col-span-2 space-y-6">
          {!selectedSchool ? (
            <div className="h-full flex flex-col items-center justify-center border-2 border-dashed border-white/5 rounded-lg py-24 text-muted-foreground">
              <LineChart className="w-12 h-12 mb-4 opacity-20" />
              <p>Select a school dossier to initialize the decoder.</p>
            </div>
          ) : (
            <>
              ...
              <Card className={cn("glass border-2", savingsPotential > 0 ? "border-success/30" : "border-destructive/30")}>
                <CardContent className="pt-6">
                  <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                    ...
                    <Button className="bg-primary text-white font-bold px-8 h-12" asChild>
