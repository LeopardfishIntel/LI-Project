...
export function LeopardfishComparisonInsights({ schools, teacherProfile }: { schools: School[], teacherProfile: TeacherProfile | null }) {
    ...
    async function handleFetchComparison() {
        if (!schools || schools.length < 2 || !teacherProfile) return;
        setLoading(true);
        setResult(null);
        const res = await getSchoolComparisonInsights(schools, teacherProfile);
        setResult(res);
        setLoading(false);
    }
            ...
            <CardContent className="flex-grow flex flex-col items-center">
                {!loading && !result && (
                    <div className="flex flex-col items-center justify-center text-center flex-grow py-8 min-h-[200px]">
                        ...
                        <Button onClick={handleFetchComparison} size="lg" disabled={!teacherProfile}>
                            <Sparkles className="w-5 h-5 mr-2" />
                            {teacherProfile ? 'Generate Comparison' : 'Profile Not Found'}
                        </Button>
                    </div>
                )}
                {loading && (
